...
{
    sjtu::vector<query_ticket_ans> vec = tts.query_station_station(query_ticket_ss_data(ui->start_by_station->currentText(), ui->end_by_station->currentText(), ui->time_by_station->currentData().toInt()));
...
{
    sjtu::vector<query_ticket_ans> vec = tts.query_city_city(quert_ticket_cc_data(ui->start_by_city->currentText(), ui->end_by_city->currentText(), ui->time_by_city->currentData().toInt()));
    for(int i = 0; i < vec.size(); ++i)
    {
        QListWidgetItem * Qlw = new QListWidgetItem(vec[i].toQString(), ui->listWidget);
    }
